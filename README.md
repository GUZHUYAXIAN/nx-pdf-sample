...
**V2 候选版**（状态更新于 2026-09-14）。已实现有界自动发现与手动选择兜底，修复选择重入、带引号环境路径和诊断留存问题；多候选选定后仍可切换，显示根目录与版本。最近一次 Release 测试（2026-09-14）200/200 通过。用户现要求不再等待本机 NX、三档 DPI、物理断网及同事异路径验收证据，继续收尾；这些项目记为“用户放行、证据未核验”，不是实测通过。未宣称 V2 全部验证完成，也未关闭验收 Issue。

用户已授权将 V2 作为独立的 GitHub Release 提供下载，使用 `v2.0.0-rc.1`
标签并标记为 Pre-release。请在 [Releases 下载页](https://github.com/GUZHUYAXIAN/nx-pdf-sample/releases)
选择 V2 的 `NX-Drawing-PDF-Exporter-v2.0.0-rc.1-win-x64.zip`；不是 GitHub 自动生成的
Source code 源码压缩包。构建提交、SHA-256 和已核验范围以该 Release 说明为准。
