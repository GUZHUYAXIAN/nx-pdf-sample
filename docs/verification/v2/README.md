...
已满足。源码已采用 MIT 许可证并公开；用户随后单独授权创建 V2 二进制 Release。
本次发布目标为 `v2.0.0-rc.1`（Pre-release），从最终提交重新构建，附带 LICENSE、
ZIP 校验和及载荷清单。最终构建提交、资产哈希和上传结果以 GitHub Release 说明为准。
以下 r2 ZIP 为历史验收候选记录；旧包保留，不冒充最终提交的构建产物。

## 候选与工作区

- 隔离目录：本项目的独立 worktree（开发机用户目录不公开）。
- 历史实施基线：`414dc5fd6f43cf2f8fa6b97812b607405221967d`，当时为 detached；审查的是
...
上述为当时的源码授权范围。后续用户明确要求 V2 直接作为新的 Release，现授权
创建对应版本标签、上传重新构建的 ZIP 和校验清单；不包含修改/关闭验收 Issue。
